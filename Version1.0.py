...
# Function to save the wordlist to a file, letting the user choose the location
def save_wordlist_to_file(wordlist, default_filename="wordlist.txt"):
    # Open a "Save As" dialog to let the user choose where to save the wordlist
    ...
    if save_path:  # If the user selects a path
        # Append the wordlist to the file without overwriting
...
def generate_and_display():
    ...
    if base_strings:
        ...
        # Save the file to a chosen or default location
        save_file_path = save_wordlist_to_file(wordlist)

        if save_file_path:
            result_text.set(f"Wordlist saved to {save_file_path}")
        else:
            result_text.set("Save operation canceled.")
    else:
